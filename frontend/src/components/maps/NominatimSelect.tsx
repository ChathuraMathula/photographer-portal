...
type NominatimSelectProps = { cityValue: string; districtValue: string; onSelect: (city: string, district: string) => void; error?: string; };

export function NominatimSelect({ cityValue, districtValue, onSelect, error }: NominatimSelectProps) {
  const { open, setOpen, searchTerm, setSearchTerm, suggestions, setSuggestions, loading, containerRef, handleTyping } = useNominatimSearch();
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      {open && (
        <NominatimDropdown
          ...
          onSelectOption={(city, dist) => { onSelect(city, dist); setOpen(false); setSearchTerm(""); setSuggestions([]); }}
        />
      )}
    </div>
  );
