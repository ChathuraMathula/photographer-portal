...
type Props<T extends LocationPickerFormValues> = {
  ...
  onCoordinatesChange: (e: React.ChangeEvent<HTMLInputElement>) => Promise<void>;
};

export function LocationPickerCoordinates<T extends LocationPickerFormValues>({ formik, geocodingStatus, onCoordinatesChange }: Props<T>) {
  return (
    <div className="space-y-2">
      ...
      />
      {geocodingStatus && <p className="text-[10px] text-zinc-550 dark:text-zinc-400 font-semibold animate-pulse mt-1">{geocodingStatus}</p>}
      <FieldError msg={(formik.touched as any).coordinates ? (formik.errors as any).coordinates : undefined} />
