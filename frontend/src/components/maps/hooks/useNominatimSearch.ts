...
export function useNominatimSearch() {
  ...
  const fetchSuggestions = async (query: string) => {
    ...
    } catch (err) { console.error("Nominatim suggestion fetch failed", err); } finally { setLoading(false); }
  };
  ...
  return { open, setOpen, searchTerm, setSearchTerm, suggestions, setSuggestions, loading, containerRef, handleTyping };
}
