  ...
  return (
    <div className="bg-blue-50/50 dark:bg-blue-950/10 p-4 rounded-xl border border-blue-100/30 dark:border-blue-900/20 space-y-3">
      <div className="flex items-center gap-1.5 text-blue-700 dark:text-blue-400">
        ...
        <span className="text-[10px] font-bold uppercase tracking-wider">Test Sandbox Cards</span>
      </div>
      <div className="grid grid-cols-2 gap-2 text-[10px] font-sans">
        {TEST_CARDS.map((card, i) => (
          ...
          >
            <span className="font-bold text-zinc-700 dark:text-zinc-300">{card.label}</span>
            <span className="font-mono text-[9px] text-zinc-450 dark:text-zinc-550 mt-0.5">{card.number}</span>
          </button>
        ))}
      </div>
    </div>
