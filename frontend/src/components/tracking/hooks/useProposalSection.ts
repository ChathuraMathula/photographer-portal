...
export function useProposalSection(reservation: TrackingReservation, selectedPkgId: string | null) {
  const getActiveDeposit = () => {
    if (!selectedPkgId || !reservation.selectedPackages) return reservation.advancePaymentPriceInCents ?? 0;
