      ...
      <div className="space-y-2">
        ...
        <OSMMapPicker
          lat={
            locationMapLink
              ? parseFloat(
                  locationMapLink.match(/q=(-?\d+\.\d+),(-?\d+\.\d+)/)?.[1] ||
                    "",
                ) || undefined
              : undefined
          }
          lon={
            locationMapLink
              ? parseFloat(
                  locationMapLink.match(/q=(-?\d+\.\d+),(-?\d+\.\d+)/)?.[2] ||
                    "",
                ) || undefined
              : undefined
          }
      ...
      {onShowMapPreviewOnBookingPageChange && (
        <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
          ...
          <button
            type="button"
            onClick={() =>
              onShowMapPreviewOnBookingPageChange(!showMapPreviewOnBookingPage)
            }
            className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${showMapPreviewOnBookingPage ? "bg-indigo-500" : "bg-zinc-300 dark:bg-zinc-600"}`}
          >
            <span
              className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${showMapPreviewOnBookingPage ? "translate-x-4" : "translate-x-0"}`}
            />
          </button>
        </div>
      )}
