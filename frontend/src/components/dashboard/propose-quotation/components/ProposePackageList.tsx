    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Label className="text-body-small-s font-semibold text-zinc-700 dark:text-zinc-300">
          Choose Package Recommendations
        </Label>
        <Button
          type="button"
          variant="outline"
      ...
      <div className="space-y-1.5 max-h-[220px] overflow-y-auto border border-zinc-200 dark:border-zinc-850 p-2.5 rounded-xl bg-zinc-50/20">
        {packages.length === 0 && !customPackage ? (
          <p className="text-body-small italic text-zinc-400">
            No packages. Create standard packages or click custom package to
            define one.
          </p>
        ) : (
          <>
            {packages.map((pkg) => (
              <PackageListItem
                key={pkg.id}
                pkg={pkg}
                isSelected={selectedPkgIds.includes(pkg.id)}
                onToggle={(c) => onTogglePackage(pkg.id, c)}
                depositValue={packageDeposits[pkg.id] ?? ""}
                onDepositChange={(v) =>
                  setPackageDeposits((prev) => ({ ...prev, [pkg.id]: v }))
                }
              />
            ))}
            {customPackage && (
              <CustomPackageListItem
                customPackage={customPackage}
                isSelected={isCustomPackageSelected}
                onToggle={setIsCustomPackageSelected}
                onRemove={() => setCustomPackage(null)}
                depositValue={customPackageDeposit}
                onDepositChange={setCustomPackageDeposit}
              />
            )}
          </>
        )}
