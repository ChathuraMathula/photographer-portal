...
export function useAdminReportsData() {
  ...
  const getPeriodDateRange = () => {
    ...
    if (period === "monthly")
      return {
        ...
        end: `${selectedYear}-${selectedMonth}-${new Date(Number(selectedYear), Number(selectedMonth), 0).getDate().toString().padStart(2, "0")}`,
      };
  ...
  const getUrl = (endpoint: string, page?: number, search?: string) => {
    ...
    if (page)
      url += `&page=${page}&limit=${endpoint === "leaderboard" ? 5 : 10}`;
    if (search) url += `&search=${encodeURIComponent(search)}`;
    if (period === "custom")
      url += `&startDate=${startDate}&endDate=${endDate}`;
    else if (period === "yearly" || period === "monthly") {
      const r = getPeriodDateRange();
      url += `&startDate=${r.start}&endDate=${r.end}`;
    }
    return url;
  };
  ...
  const fetchData = async (
    ...
    try {
      const res = await fetch(getUrl(endpoint, page, search), {
        credentials: "include",
      });
  ...
  useEffect(() => {
    ...
    fetchData(
      "data",
      setReportData,
      (b) => (reportData === null ? setLoading(b) : setRefreshing(b)),
  ...
  const handleDownload = async (
    ...
    setDownloading(true);
    try {
      const res = await fetch(getUrl(`pdf/${type}`), {
        credentials: "include",
      });
      if (!res.ok) throw new Error("Failed");
      const a = document.createElement("a");
      a.href = window.URL.createObjectURL(await res.blob());
      a.download = `system_${type}_report_${period}_${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  return {
    ...
    handleDownloadFinancial: () =>
      handleDownload("financial", setDownloadingFinancial),
    handleDownloadBookings: () =>
      handleDownload("bookings", setDownloadingBookings),
    handleDownloadLocation: () =>
      handleDownload("location", setDownloadingLocation),
    bookingsData,
    bookingsPage,
    setBookingsPage,
    bookingsLoading,
