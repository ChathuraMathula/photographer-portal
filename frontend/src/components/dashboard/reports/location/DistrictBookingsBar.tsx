...
// Distinct palette for event-type segments
const SEGMENT_COLORS = [
  "#6366f1", "#10b981", "#f59e0b", "#3b82f6",
  "#ec4899", "#14b8a6", "#f97316", "#8b5cf6",
];

export function DistrictBookingsBar({ data }: Props) {
        ...
        return (
          ...
          >
            <div className="flex justify-between items-center mb-1 text-xs">
              <span className="font-semibold text-zinc-800 dark:text-zinc-200 truncate pr-3">
                {d.district}
              </span>
            ...
            <div className="relative h-3 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
              {eventTypeKeys.map((et, etIdx) => {
                ...
                return (
                  <div
                    key={et}
                    title={`${et}: ${d.eventTypes[et]}`}
                    className="absolute top-0 h-full rounded-full transition-all duration-500"
                    style={{
                      width: `${segPct}%`,
                      left: `${eventTypeKeys
                        .slice(0, etIdx)
                        .reduce((acc, k) => acc + (d.eventTypes[k] / d.count) * barPct, 0)}%`,
                      backgroundColor: SEGMENT_COLORS[etIdx % SEGMENT_COLORS.length],
            ...
            {/* Tooltip-style event breakdown on hover */}
            {isHovered && eventTypeKeys.length > 0 && (
              <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5">
                {eventTypeKeys.map((et, etIdx) => (
                  <span key={et} className="text-[10px] text-zinc-500 flex items-center gap-1">
                    <span
                      className="inline-block h-2 w-2 rounded-full"
                      style={{ backgroundColor: SEGMENT_COLORS[etIdx % SEGMENT_COLORS.length] }}
                    />
                    {et} ({d.eventTypes[et]})
                  </span>
                ))}
