...
export function LocationAnalyticsSection({
  ...
  const { mapPoints, districtStats, cityStats, insights, eventTypes } =
    useMemo(() => {
      return {
        mapPoints: buildMapPoints(rawBookings),
        districtStats: buildDistrictStats(rawBookings),
        cityStats: buildCityStats(rawBookings),
        insights: buildLocationInsights(
          rawBookings,
          buildDistrictStats(rawBookings),
          buildCityStats(rawBookings),
          buildMapPoints(rawBookings),
        ),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="text-body-base-bold font-bold text-zinc-900 dark:text-white">
            {title}
          </h3>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card className="border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm rounded-xl">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Top 10 districts ranked by total bookings
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm rounded-xl">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Top 8 cities ranked by booking count
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {districtStats.length > 0 && eventTypes.length > 1 && (
        <Card className="border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm rounded-xl">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-xs">
              Visual breakdown of how event categories are distributed across
              regions
            </CardDescription>
          </CardHeader>
          <CardContent>
            <EventTypeByDistrictChart
              data={districtStats}
      ...
      <Card className="border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm rounded-xl overflow-hidden">
        <CardHeader className="pb-3">
          ...
          <CardDescription className="text-xs">
            Exact pin locations extracted from Google Maps coordinates stored
            per booking. Markers are colour-coded by event type and clustered by
            proximity.
          </CardDescription>
        </CardHeader>
