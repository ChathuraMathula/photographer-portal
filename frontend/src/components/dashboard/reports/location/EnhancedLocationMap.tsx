...
const ET_MARKER_COLORS: Record<string, string> = { Wedding: "#ec4899", Portrait: "#6366f1", Corporate: "#3b82f6", "Birthday Party": "#f59e0b", Graduation: "#10b981", Engagement: "#f97316", Maternity: "#8b5cf6", Other: "#a1a1aa" };
function pickColor(et: string): string { return ET_MARKER_COLORS[et] || "#6366f1"; }

function buildMapHtml(points: MapPoint[]): string {
  const serialized = JSON.stringify(points.map((p) => ({ ...p, color: pickColor(p.eventType) })));
  return `<!DOCTYPE html>
...
export function EnhancedLocationMap({ points }: { points: MapPoint[] }) {
  if (points.length === 0) return (
    <div className="h-64 flex flex-col items-center justify-center border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50 dark:bg-zinc-950 text-zinc-400 text-xs p-4 text-center gap-1.5"><Map className="h-6 w-6 text-zinc-350 dark:text-zinc-550" /><span className="font-semibold text-zinc-600 dark:text-zinc-350">No exact coordinates available</span><span className="text-[10px] text-zinc-450 dark:text-zinc-500">Bookings with a Google Maps link containing coordinates will appear here.</span></div>
  );
  return <div className="overflow-hidden border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm"><iframe title="Booking Location Map" style={{ height: "420px", width: "100%", border: 0 }} srcDoc={buildMapHtml(points)} sandbox="allow-scripts" /></div>;
}
