...
export function useFloatingChatDrag(
  ...
  const dragRef = useRef<{ startX: number; startY: number; buttonX: number; buttonY: number; lastX: number; lastY: number } | null>(null);
  ...
  const handleDragMove = (clientX: number, clientY: number) => {
    ...
    const x = Math.max(8, Math.min(dragRef.current.buttonX + dx, window.innerWidth - 64));
    const y = Math.max(8, Math.min(dragRef.current.buttonY + dy, window.innerHeight - 64));
  ...
  const startDrag = (clientX: number, clientY: number, button: HTMLButtonElement) => {
    const rect = button.getBoundingClientRect();
    button.style.transition = "none";
    dragRef.current = { startX: clientX, startY: clientY, buttonX: rect.left, buttonY: rect.top, lastX: rect.left, lastY: rect.top };
    isDraggingRef.current = false;
  };

  const endDrag = () => {
    if (buttonRef.current) buttonRef.current.style.transition = "";
    if (dragRef.current) setPosition({ x: dragRef.current.lastX, y: dragRef.current.lastY });
    setTimeout(() => {
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const onMouseMove = (me: MouseEvent) => handleDragMove(me.clientX, me.clientY);
    const onMouseUp = () => {
      endDrag();
  ...
  const handleTouchStart = (e: React.TouchEvent<HTMLButtonElement>) => {
    ...
    const onTouchMove = (te: TouchEvent) => handleDragMove(te.touches[0].clientX, te.touches[0].clientY);
    const onTouchEnd = () => {
      endDrag();
