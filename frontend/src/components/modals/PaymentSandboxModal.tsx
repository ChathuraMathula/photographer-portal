...
export function PaymentSandboxModal({
  ...
  return (
    <AlertDialog
      open={open}
      onOpenChange={(v) => !v && paymentStatus !== "processing" && onClose()}
    >
        ...
        {paymentStatus === "processing" || paymentStatus === "success" ? (
            ...
            {paymentStatus === "processing" ? (
              ...
              <div className="animate-in zoom-in-95 duration-300 flex flex-col items-center space-y-3">
                ...
                <p className="text-body-small text-zinc-500 dark:text-zinc-400">
                  {isBalancePayment
                    ? "Your remaining balance is settled."
          ...
          <div className="p-6 space-y-5 flex-1 overflow-y-auto custom-scrollbar">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                Pay LKR {chargeLkr.toLocaleString()} &amp;{" "}
                {isBalancePayment ? "Settle Balance" : "Confirm Booking"}
              </button>
            </form>
          </div>
        )}
