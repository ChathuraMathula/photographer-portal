...
export function ManualBookingLocationFields({
  formik,
  allowedEventTypes = [],
  ...
  const {
    geocodingStatus,
    handleCityDistrictSelect,
    handleCoordinatesChange,
    handleMapChange,
  } = useManualBookingGeocode(formik);

  const getMapCoords = () => {
    const m = formik.values.locationMapLink?.match(
      /q=(-?\d+\.\d+),(-?\d+\.\d+)/,
    );
    return {
      lat: m ? parseFloat(m[1]) : undefined,
      lon: m ? parseFloat(m[2]) : undefined,
    };
  };
    ...
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            className={`h-11 rounded-xl border-zinc-200 focus:ring-primary-dark ${formik.touched.location && formik.errors.location ? "border-red-500" : ""}`}
          />
          <FieldError
      ...
      <div className="space-y-2">
        ...
        <NominatimSelect
          ...
          onSelect={handleCityDistrictSelect}
          error={
        ...
        <FieldError
          msg={
            (formik.touched.city && formik.errors.city) ||
            (formik.touched.district && formik.errors.district)
              ? formik.errors.city || formik.errors.district
              : undefined
          }
        />
      </div>
      <div className="space-y-2">
        ...
        <Input
          id="mb-coordinates"
          placeholder="Paste exact coordinates..."
          value={formik.values.coordinates}
          onChange={(e) => handleCoordinatesChange(e.target.value)}
          className={`h-11 rounded-xl border-zinc-200 ${formik.touched.coordinates && formik.errors.coordinates ? "border-red-500" : ""}`}
        />
        {geocodingStatus && (
          <p className="text-[10px] text-zinc-550 font-semibold animate-pulse mt-1">
            {geocodingStatus}
          </p>
        )}
      </div>
      <div className="space-y-2">
        ...
        <OSMMapPicker
          lat={getMapCoords().lat}
          lon={getMapCoords().lon}
        ...
        {formik.values.locationMapLink && (
          <p className="text-[10px] text-zinc-400 font-medium truncate mt-1">
            Generated Link:{" "}
            <span className="font-mono">{formik.values.locationMapLink}</span>
          </p>
        )}
      </div>
    </>
