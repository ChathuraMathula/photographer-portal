    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar text-left">
          <BookingDetailsMeta
            ...
            handleCopyId={state.handleCopyId}
          ...
          {reservation.customerNotes && (
            <div className="space-y-1.5 border-t border-zinc-150 dark:border-zinc-800/80 pt-4">
              ...
              <p className="text-body-small italic text-zinc-650 dark:text-zinc-350 bg-zinc-50/50 dark:bg-zinc-950/20 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/80">
                "{reservation.customerNotes}"
              </p>
            </div>
          )}
        ...
        <div className="border-t px-6 py-4 bg-zinc-50/50 dark:bg-zinc-950/20 dark:border-zinc-800 grid grid-cols-2 gap-3 shrink-0">
          <Button
            ...
            variant="outline"
            onClick={onClose}
            className="btn btn-secondary btn-modal h-11 px-6 text-body-small-s cursor-pointer"
          >
            Close Dialog
          </Button>
          <Button
            ...
            className="btn btn-primary btn-modal h-11 px-6 text-body-small-s shadow-sm font-semibold cursor-pointer"
          >
            View Reservation
          </Button>
        </div>
      ...
      {state.showCashConfirm && (
        <ConfirmationModal
          ...
          onConfirm={async () => {
            await state.handleLogCashPayment();
            state.setShowCashConfirm(false);
          }}
