  ...
  if (invoices.length === 0) {
    return (
      <div className="text-center py-12 bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-xl">
        <p className="text-body-small text-zinc-400 italic">No generated invoices found.</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left text-body-small border-collapse">
          <thead>
            <tr className="bg-zinc-50/50 dark:bg-zinc-950/40 text-zinc-550 border-b border-zinc-100 dark:border-zinc-850">
              <th className="px-6 py-3.5 font-semibold text-xs uppercase tracking-wider">Invoice No.</th>
              <th className="px-6 py-3.5 font-semibold text-xs uppercase tracking-wider">Client</th>
              <th className="px-6 py-3.5 font-semibold text-xs uppercase tracking-wider">Event Details</th>
              <th className="px-6 py-3.5 font-semibold text-xs uppercase tracking-wider">Amount Settled</th>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-805">
            {invoices.map((inv) => {
              ...
              return (
                <tr key={resId} className="hover:bg-zinc-50/30 dark:hover:bg-zinc-950/20 transition-colors">
                  <td className="px-6 py-4 font-semibold text-zinc-900 dark:text-white">
                    INV-{shortId}
                  </td>
                  <td className="px-6 py-4">
                    <p className="font-semibold text-zinc-700 dark:text-zinc-300">
                      {inv.reservation.customer.firstName} {inv.reservation.customer.lastName}
                    </p>
                  ...
                  <td className="px-6 py-4 text-zinc-500 dark:text-zinc-400">
                    <p className="font-semibold text-zinc-700 dark:text-zinc-305">{inv.reservation.eventType}</p>
