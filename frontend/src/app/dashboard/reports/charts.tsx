...
export function RevenueAreaChart({ data }: { data: ChartDataPoint[] }) {
  ...
  const getY = (val: number) =>
    padding + chartHeight - (val / maxVal) * chartHeight;
      ...
      {hoveredIdx !== null && (
        ...
        >
          ...
          <div className="font-bold">
            LKR {data[hoveredIdx].revenueLkr.toLocaleString()}
          </div>
          <div className="text-[10px] text-zinc-350 dark:text-zinc-550">
            {data[hoveredIdx].bookings} Booking(s)
          </div>
        </div>
      )}
    </div>
    ...
    <div className="flex flex-col xl:flex-row items-center justify-center gap-6 w-full py-2">
      ...
      <div className="relative w-40 h-40 shrink-0">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center font-sans">
          <span className="text-title-large font-extrabold text-zinc-800 dark:text-white leading-none">
            {total}
          </span>
        ...
        return (
          <div key={i} className="space-y-1.5">
            <div className="flex justify-between items-center text-xs">
              <span className="font-bold text-zinc-800 dark:text-zinc-200 truncate pr-4">
                {pkg.name}
              </span>
              <span className="text-zinc-500 shrink-0 font-semibold">
                LKR {pkg.revenueLkr.toLocaleString()} ({pkg.count} Booking
                {pkg.count !== 1 ? "s" : ""})
              </span>
            </div>
