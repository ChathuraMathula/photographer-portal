...
function waitForPort(port, timeoutMs) {
  return new Promise(function (resolve, reject) {
    ...
    function tryConnect() {
        ...
        return reject(
          new Error(
            "Backend did not become ready on port " +
              ...
              " within " +
              timeoutMs / 1000 +
              "s. Check backend logs for errors.",
          ),
        );
      }
      attempt++;
      log(
        "WAIT",
        "Checking backend on port " + port + " (attempt " + attempt + ")...",
      );
...
async function main() {
  ...
  log(
    "WAIT",
    "Waiting for backend to become ready on port " + BACKEND_PORT + "...",
  );
  try {
