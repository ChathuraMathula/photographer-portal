  ...
  try {
    ...
    for (const line of lines) {
      ...
      for (const port of ports) {
        if (localAddress.endsWith(`:${port}`) || localAddress.includes(`:${port}`)) {
          // Double check to make sure it's actually the port (e.g. not part of IP)
          const portStr = `:${port}`;
          const isExactPort = localAddress.endsWith(portStr) || localAddress.split(':').pop() === String(port);
          if (isExactPort) {
    ...
    for (const pid of pidsToKill) {
      console.log(`[Port Cleanup] Killing process with PID ${pid} occupying dev port...`);
      try {
    ...
    console.error('[Port Cleanup] Failed to run netstat or taskkill:', e.message);
  }
} else {
  // macOS / Linux
  for (const port of ports) {
