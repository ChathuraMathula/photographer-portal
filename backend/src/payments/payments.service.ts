...
export class PaymentsService {
  ...
  async processPayment(dto: ProcessPaymentDto) {
    ...
    if (
      ...
      throw new BadRequestException('Payment deadline has expired (24 hours exceeded). Please contact the photographer.');
    }
    ...
    const { status, errorMsg, resolvedCardBrand } = await this.stripeHelper.simulateStripeCharge(dto.cardNumber);
    ...
    if (isBalancePayment) {
      ...
      if (selectedPkg) {
        if (selectedPkg.customDepositAmountInCents !== undefined && selectedPkg.customDepositAmountInCents !== null) {
          ...
          depositAmountInCents = Math.round((selectedPkg.priceInCents * (selectedPkg.depositValue || 0)) / 100);
        }
      }
    ...
    if (!isBalancePayment) {
      ...
      if (conflicts.length > 0) {
        throw new BadRequestException('This time slot has already been booked and confirmed by another user in the meantime.');
      }
    ...
    if (isBalancePayment) {
      await this.stripeHelper.sendInvoiceAndNotify(reservation, [...successfulPayments, payment], this.profileRepository);
    } else {
    ...
    return {
      ...
      transactionId,
      message: isBalancePayment ? 'Remaining balance processed successfully' : 'Payment processed and reservation confirmed successfully',
    };
  }
  ...
  async getPhotographerTransactions(
    ...
    query: { page?: number; limit?: number; search?: string; status?: string; method?: string; } = {},
  ) {
    ...
    const statsQb = this.paymentRepository.createQueryBuilder('payment')
